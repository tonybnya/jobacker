...
export function getProfile(token: string, userId: string) {
  return dbFetch<Profile[]>(`/profiles?select=*&id=eq.${userId}`, token).then(
    (r) => ({ data: r.data?.[0] ?? null, error: r.error }),
  );
}

export function createProfile(token: string, userId: string, email: string) {
  return dbFetch<Profile[]>("/profiles", token, {
    method: "POST",
...
export function updateProfile(token: string, userId: string, updates: Partial<Pick<Profile, "full_name" | "phone" | "location" | "resume_pdf_url" | "resume_text">>) {
  return dbFetch<Profile[]>(`/profiles?id=eq.${userId}`, token, {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
